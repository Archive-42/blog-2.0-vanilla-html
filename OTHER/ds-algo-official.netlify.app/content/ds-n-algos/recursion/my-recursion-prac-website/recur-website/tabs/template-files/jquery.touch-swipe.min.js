...
!(factory => {
  ...
  function TouchSwipe(element, options) {
    ...
    function touchEnd(jqEvent) {
      const event = jqEvent.originalEvent ? jqEvent.originalEvent : jqEvent, touches = event.touches;
      if (touches) {
    ...
    function validatePinch() {
      const hasCorrectFingerCount = validateFingers(), hasEndPoint = validateEndPoint(), hasCorrectDistance = validatePinchDistance();
    ...
    function updateFingerData(evt) {
      const id = void 0 !== evt.identifier ? evt.identifier : 0;
      let f = getFingerData(id);
      return (
    ...
    function calculateTouchesDistance(startPoint, endPoint) {
      const diffX = Math.abs(startPoint.x - endPoint.x), diffY = Math.abs(startPoint.y - endPoint.y);
    ...
    function calculateAngle(startPoint, endPoint) {
      ...
      let angle = Math.round((180 * r) / Math.PI);
    ...
    var LEAVE_EV = useTouchEvents
      ? SUPPORTS_POINTER
        ? "mouseleave"
        : null
      : "mouseleave";

    var CANCEL_EV = SUPPORTS_POINTER
      ? SUPPORTS_POINTER_IE10
        ? "MSPointerCancel"
        : "pointercancel"
      : "touchcancel";
  ...
  var SUPPORTS_POINTER_IE10 =
    window.navigator.msPointerEnabled &&
    !window.navigator.pointerEnabled &&
    !SUPPORTS_TOUCH;

  var SUPPORTS_POINTER =
  ...
  ($.fn.swipe = function (method) {
    const $this = $(this), plugin = $this.data(PLUGIN_NS);
